...
do
    ...
    expectedProgramOutput=$(cat "${currTestName}/expectedOutput.txt")
    expectedExitMessage=$(cat "${currTestName}/expectedExitMessage.txt")
    actualExitMessage=$(eval "$TESTING_PROGRAM_NAME $inputFileData")
    actualProgramOutput=""

    if [ -f "actualOutput.txt" ]
    then
        actualProgramOutput=$(cat actualOutput.txt)
        rm actualOutput.txt
    fi

    if [[ "$expectedProgramOutput" != "$actualProgramOutput" || "$expectedExitMessage" != "$actualExitMessage" ]]
    then
        echo -e "[Test '${currTestName}']: ${FAILED_TEST_COLOR} FAILED ${NORMAL_TEST_COLOR}"
        echo "    Expected output: $expectedProgramOutput"
        echo "    Actual output: $actualProgramOutput"
